...
namespace ApiApplication.UseCases.Commands.Reservations.ConfirmReservation
{
    public class ConfirmReservationCommandHandler : IRequestHandler<ConfirmReservationCommand>
    {
        ...
        public async Task<Unit> Handle(ConfirmReservationCommand command, CancellationToken cancel)
        {
            ...
            try
            {
                await ticketsRepository.UpdateAsync(reservation, cancel);
                return Unit.Value;
            }
            catch (Exception e)
            {
        ...
        private async Task<TicketEntity> GetReservationIfExistsOrThrowException(ConfirmReservationCommand command, CancellationToken cancel)
        {
